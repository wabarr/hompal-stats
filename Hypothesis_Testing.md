...
We want to test the hypothesis of a relationship between sex and femoral head diameter. To do this, we first create a statistical ***null hypothesis***.
...
For baboons, use T-test to compare means.
...
When you find statistical significance, you have to ask (how big is the effect? should I care?)
...
When we compute the p value, we will ask how many  differences computed on the randomized data are as extreme as our actual mean difference. 
...
If true, then our F ratio tends to be about 1. Larger and larger values of F are less and less likely to be observed under the null hypothesis.  
...
fdist <- qplot(x=xrange, y=yvals, geom="line") + 
    theme_bw(20) + 

...
```{r cache=TRUE, echo=FALSE, fig.width=4.5, fig.height=5, warning=F}
x <- runif(50, 0, 2.5)
y <- x^3 - 3*x
...
## Choosing a General Linear Model {.build}
...
> - `heads` - [Howell Craniometric Data](http://hompal-stats.wabarr.com/datasets/Howell_craniometry.txt)
> - `fossils` - [Fossil Data](http://hompal-stats.wabarr.com/datasets/Gordon_2008_cranial.txt)
...
*  add a new layer with the fossil data, being sure to differentiate them visually
    *  add the fossil regression line to the plot, using a diffent kind of line (e.g. dashed, or a different color)

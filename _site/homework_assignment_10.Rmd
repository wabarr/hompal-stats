...
B. Select the following columns from the data: `Taxon`, `B`, `DistRad`, and `Habitat`.  Create a new dataframe with the `Taxon`, the `Habitat` and  species means for `B` and `DistRad`.
...
F.  Perform PGLS to test the hypothesis that `log(B)` is a function of `log(DistRad)`, while controlling for phylogenetic signal. Be sure to include the geometric mean of these measurements (square root of their product) as a covariate to control for body size. Is this hypothesis supported?

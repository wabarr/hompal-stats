Note from Spring 2015:  Students reported spending >20 hours on this assignment. 
...
c.  Write a new function called `geomean()` that accepts a vector (of any length) as an argument and returns the geometric mean of that vector. 
    *  **Note:** Your function must deal properly with missing values (NA) as arguments.
...
b. use `lapply()` to loop over each column of the crania dataframe. For each column, apply a custom-written function called `myFUN`.  `myFUN` should test if the column contains numeric data (using the `is.numeric()` function).  If it is numeric, calculate and return the mean value, being careful to remove NA values.  If the data are not numeric, return the string "Not Numeric". 

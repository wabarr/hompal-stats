...
You call a function by typing its name, followed by parentheses containing any **arguments** necessary. Arguments are the way you pass data and/or options to a function. For example, the `paste()` function simply pastes together any arguments you privide into a single text string.
...
1.  Use `rnorm` to create a vector called `grades` representing student grades from an Anthropology 101 course with  200 students, mean grade of 68%, and a standard deviation of 10. 

2.  Apply a curve of 7% to the class grades in order to raise the mean to to 75%.  Save this to a new variable called `curvedgrades`

3.  Make a histogram of `curvedgrades` using the `hist()` function. Note: You can see all the arguments and default values for any function using the `?` operator like this `?hist`.
...
Read in the file called [femur_lengths.txt](datasets/femur_lengths.txt). Save to an object called `femora`.

## Data Output

Usually, I would recommend **not** writing out modified data frames to text files.  It is far better to have a single input file, and to do all necessary manipulations in your saved R script file. But, if you have huge datasets or need to use another statistical program, it is sometimes necesary to save a dataframe outside of R as a text file.  You can do this with `write.table()`

...
```{r}
summarise(loans, mean(SubsidizedDL, na.rm=TRUE))
```
...
summarise(groupedLoans, mean(SubsidizedDL, na.rm=TRUE))
```
...
**Note:** If you like this idea of chaining operations, this actually comes from a separate package called `magrittr`, which `dplyr` relies on.  If you want you can use this in your non `dplyr` code as well. Notice that in the example below, if you are piping to a 
